...
(document.getElementById("searchLastNameBtn") as HTMLButtonElement).onclick = () => {
  const keyword = (document.getElementById("searchLastName") as HTMLInputElement).value;
  const results = manager.findStudentsByLastName(keyword);
  showList<Student>(results);
  alert(`ผลการค้นหา: ${results.length} คน`);
};
